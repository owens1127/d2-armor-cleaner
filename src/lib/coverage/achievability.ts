...
export function prioritiesCoveredByArchetypeIntrinsics(
  archetype: Archetype,
  priorities: Stat[],
): boolean {
  const [primary, secondary] = ARCHETYPE_STATS[archetype];
  const intrinsic = new Set<Stat>([primary, secondary]);
...
export function isBestTierLoadoutPiece(item: ArmorPiece, priorities: Stat[]): boolean {
  ...
  return computeOptimalRollShapes(priorities).some((shape) =>
